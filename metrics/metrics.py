...
def cce_flatt(void_class, weights_class):
    def categorical_crossentropy_flatt(y_true, y_pred):
        ...
        y_pred = K.reshape(y_pred, (-1, shp_y_pred[3]))  # go back to b01,c
        ...
        if data_format == 'channels_first':
            ...
            y_true = tf.one_hot(y_true, K.shape(y_pred)[-1], on_value=1,
                                off_value=0, axis=None, dtype=None, name=None)
...
def ConfusionMatrix(n_classes, void_labels):
    def conf_matrix(y_true, y_pred):
        # Flatten
        y_true = K.cast(K.flatten(y_true), 'int32')
        y_pred = K.flatten(K.argmax(y_pred, axis=channel_idx()))
        
        cm = tf.confusion_matrix(y_true, y_pred, num_classes=n_classes,
...
def IoU(n_classes, void_labels):
    def IoU_flatt(y_true, y_pred):
        ...
        # Simplification:
        y_pred = K.argmax(y_pred, axis=channel_idx())
        y_pred = K.flatten(y_pred)  # b,01 -> b01
        y_true = K.cast(K.flatten(y_true), 'int32')  # b,01 -> b01

        # # Make the last channel to be the number of classes
        # if data_format == 'channels_first':
        #     y_pred = K.permute_dimensions(y_pred, (0, 2, 3, 1))

        # # Convert prediction to a vector b01
        # shp_y_pred = K.shape(y_pred)
        # y_pred = K.reshape(y_pred, (shp_y_pred[0]*shp_y_pred[1]*shp_y_pred[2],
        #                    shp_y_pred[3]))  # go back to b01,c
        ...
        for i in range(n_classes):
            ...
            if data_format == 'channels_first':
                ...
                I_i = K.sum(K.cast(y_true_i, 'float32') * K.cast(y_pred_i, 'float32'))
                sum_I = sum_I + I_i
            out['I'+str(i)] = I_i
            out['U'+str(i)] = U_i
...
def YOLOLoss(input_shape=(3, 640, 640), num_classes=45,
             priors=[[0.25, 0.25], [0.5, 0.5], [1.0, 1.0], [1.7, 1.7], [2.5, 2.5]],
             max_truth_boxes=30, thresh=0.6, object_scale=5.0,
             noobject_scale=1.0, coord_scale=1.0, class_scale=1.0):
    """
    YOLO loss function
    ...
    def _YOLOLoss(y_true, y_pred, name=None, priors=priors):
        ...
        intersect = tf.multiply(intersect_wh[:, :, :, 0],
                                intersect_wh[:, :, :, 1])
...
def YOLOMetrics(input_shape=(3,640,640),num_classes=45,priors=[[0.25,0.25], [0.5,0.5], [1.0,1.0], [1.7,1.7], [2.5,2.5]],max_truth_boxes=30,thresh=0.6,nms_thresh=0.3):
    ...
    def _YOLOMetrics(y_true, y_pred, name=None):
        ...
        return {'avg_iou' : tf.reduce_mean(avg_iou),
                'avg_recall' : tf.reduce_mean(avg_recall)}
