...
def buildUnet(nb_in_channels, dropout, input_var=None,
    ...
    # Load parameter values into the net
    with np.load(path_unet) as f:
        param_values = [f['arr_%d' % i] for i in range(len(f.files))]

    nlayers = len(lasagne.layers.get_all_params(net_final))

    lasagne.layers.set_all_param_values(net_final, param_values[:nlayers])
